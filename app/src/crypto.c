...
void zeroize_sr25519_signdata(void) { explicit_bzero(sr25519_signature, sizeof(sr25519_signature)); }
...
catch_cx_error:
    if (error == zxerr_ok) {
        sign_sr25519_phase2((const uint8_t *)privateKeyData, (const uint8_t *)pubkey, NULL, 0, data, len,
                            sr25519_signature + 1);
    } else {
